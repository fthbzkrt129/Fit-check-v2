...
import { OutfitLayer } from '@/lib/kombin/types';
          ...
          >
            <div className="flex items-center overflow-hidden">
                ...
                <div className="truncate">
                  <span className="block font-semibold text-gray-800 truncate" title={layer.garment?.name}>
                    {layer.garment ? layer.garment.name : 'Base Model'}
                  </span>
                  {layer.category !== 'base' && (
                    <span className="block text-xs text-gray-500 truncate">
                      {layer.category === 'top' ? 'Üst Giyim' : layer.category === 'bottom' ? 'Alt Giyim' : layer.category === 'footwear' ? 'Ayakkabı' : 'Aksesuar'}
                      {layer.topLength ? ` • ${layer.topLength === 'crop' ? 'Crop' : layer.topLength === 'waist' ? 'Bel' : layer.topLength === 'hip' ? 'Kalça' : 'Tunik'}` : ''}
                    </span>
                  )}
                </div>
            </div>
